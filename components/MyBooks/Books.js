...
const Books = ({ isAudio }) => {
  ...
  useEffect(() => {
    (async () => {
      setPage(1);
      const response = isAudio ? await BooksService.getMyAudioBooks(router.query) : await BooksService.getMyBooks(router.query);
  ...
  useEffect(() => {
    if (page > 1) {
      setIsLoading(true);
      (async () => {
        const response = isAudio ? await BooksService.getMyAudioBooks({ page, ...router.query }) : await BooksService.getMyBooks({ page, ...router.query });
